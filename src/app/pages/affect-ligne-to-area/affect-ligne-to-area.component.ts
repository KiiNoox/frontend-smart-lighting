...
@Component({
  selector: 'app-affect-ligne-to-area',
  templateUrl: './affect-ligne-to-area.component.html',
  styleUrls: ['./affect-ligne-to-area.component.css']
})
export class AffectLigneToAreaComponent implements OnInit {
 ligne: Ligne;
  data = [];
  keyword = 'name';
  @Input() public id;
  ...
  affect() {
    this.areaForm1.controls['Ligne'].setValue(this.id);
    console.log(this.areaForm1.value);

    this.ligne = this.id;
    console.log(this.ligne);
   this.ligneService.affectLignetoArea( this.idArea, this.areaForm1.value ).subscribe();
  }
}
