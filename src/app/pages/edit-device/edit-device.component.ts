...
export class EditDeviceComponent implements OnInit {
  ...
  constructor(private dialog: NgbModal , public areaService: AreaService ,
              public ligneService: LignesService , public devicesService: DevicesService , @Inject(LOCAL_STORAGE) private storage: WebStorageService) { }
  ...
  ngOnInit(): void {
    ...
    this.devicesService.GetAllDevice(decodedHeader.users._id).subscribe(data => {
      for ( let i = 0; i < data.length ; i++) {
        this.marker5(data[i].Lng , data[i].Lat , data[i].name)
      }
    });
  ...
  marker5(a: number , b: number , c: String) {
  ...
  selectEvent(item) {
    ...
    this.ligneService.getLigneByAreaId(item._id).subscribe(data => {
      this.data2 = data;
    });
  }
  ...
  EditDevice() {
    ...
    this.devicesService.EditDevice(this.areaForm1.value , this.id._id).subscribe();
